...
function taskCreater() {
  for (let index in textArray) {
    ...
    task[`${index}`].div.circle.appendChild(
      task[`${index}`].div.circle.smallCircle
    );
    // check-icon
    task[`${index}`].div.circle.checkIcon = document.createElement("img");
    task[`${index}`].div.circle.checkIcon.setAttribute(
      "src",
      "./assets/icon-check.svg"
    );
    task[`${index}`].div.circle.appendChild(
      task[`${index}`].div.circle.checkIcon
    );
    ...
    task[`${index}`].div.cross.image.setAttribute(
      "src",
    ...
    task[`${index}`].div.cross.hoverImage.setAttribute(
      "src",
      "./assets/cross-hover.svg"
    );
    task[`${index}`].div.cross.appendChild(
      task[`${index}`].div.cross.hoverImage
    );
...
function addCircleClickListener(task, key) {
  task[key].div.circle.addEventListener("click", () => {
    if (taskState[key] == "active") {
      ...
      task[
        key
      ].div.info.innerHTML = `<del>${task[key].div.info.textContent}</del>`;
